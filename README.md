...
The training-video cutaway at `/slides#blackout` fully downloads `blackout-training.mp4` before playback, then holds black for three seconds. The video and captions stay inside a centered 16:9 frame. Captions follow the spoken language (English and French) in `blackout-training.vtt`. Tape distortion starts at 36.5 seconds and cuts to silence and black at 39.5 seconds; the presenter advances manually afterward. Re-entering the slide replays it. Rebuild the processed copy with `python scripts/prepare_blackout_video.py <source-video>`; the original file is preserved.
